...
\maketitle
\available[Journal]{https://github.com/jsimons8/lyx-paper}
...
We check that citations work: \citet{MS15} and one include one more:
\citet{M14}.

We can also cross-reference using the `xr' package: our equation in
the main document is (\ref{eq:line}) as long as we keep the relevant
`paper.aux' file in the same folder. The following command in the
preamble : `\textbackslash externaldocument\{paper\}' where `paper'
must be the title of the main document.
