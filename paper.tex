...
\documentclass[american,letterpaper,11pt,leqno]{article}
...
\begin{document}
\title{My paper}
\author{First Author, Second Author\thanks{First Author: First University. Second Author: Second University.
We thank colleagues for helpful comments and discussions. This work
was supported by a grant {[}grant number{]}; another grant {[}grant
number{]}; and a foundation.\}}}
\date{June 08, 2025}
...
\subfloat[Left panel]{\includegraphics[scale=0.2,page=1]{../latex-paper-lyx/figures}

}\hfill{}\subfloat[Right panel]{\includegraphics[scale=0.2,page=2]{../latex-paper-lyx/figures}

}\caption{Panel figure.}
...
We make sure that citations work like so \citet{MS15} and one more:
\citet{M14}. The bibliography style is `chicago' so that the document
compiles under `pdflatex.' If one wishes to use the original `bibliography.bst,'
plain LaTeX compilation should be used.

\bibliographystyle{chicago}
\bibliography{../latex-paper-lyx/bibliography}
...
This is an in-document appendix. Some journals offer authors to upload
a separate appendix which is only published online. For a document
that produces a separate appendix, please see `appendix.lyx.`
\end{document}
